...
from GAT.gat_hwithBAM_classification_citeseer import *
...
parser.add_argument('--nb_node_sample', type=int, default=250, help='number of sample nodes')
parser.add_argument('--k', type=float, default=0.6, help='sample distribution')
...
parser.add_argument('--att_kl', type=float, default=0.5,
...
Setting.H = [16]  # len(np.unique(train_label))
...
if args.sub_sample:
    ...
    replace = False
    eta = 0.0000009# depends on nb_node_samples
...
data_LH = 0.000007 * tf.reduce_sum(X_VN*log_max(tf.matmul(Phi_1, Theta_1)) - tf.matmul(Phi_1, Theta_1) - tf.lgamma(X_VN + 1))  # / tf.cast(Batch_Size, tf.float32)
...
for sweepi in range(Setting.SweepTimes):
    ...
    if np.mod(sweepi, 1) == 0:
        ...
        outs = sess.run([Label_acc, Label_loss, data_LH, graph_LH, KL_loss, LossL2],
                         feed_dict=feed_dict)

        [Label_acc_t, Label_loss_t, data_LH_t, graph_LH_t, kl_loss, lossL2] = outs


        print('Epoch: {:3d}'.format(sweepi))

        print('Ac: {:<8.4f}, Label_LH: {:<8.4f}, Data_LH: {:<8.4f}, Graph_LH: {:<8.4f}, KL_loss: {:<8.4f}, lossl2: {:<8.4f}'.format(Label_acc_t, Label_loss_t, data_LH_t, graph_LH_t, kl_loss, lossL2))
...
#     tl[i] = tmp + tl[i]
#     tmp = tl[i]
# f = open('./citeseer_acc/WGAAE_noscale_Phi2.pkl', 'wb')
# dict = {'tl': tl, 'val': val}
# pickle.dump(dict, f)
# f.close()