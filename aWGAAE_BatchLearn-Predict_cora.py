...
from GAT.gat_hwithBAM_prediction_cora import *
...
parser.add_argument('--nb_node_sample', type=int, default=100, help='number of sample nodes')
parser.add_argument('--k', type=float, default=0.8, help='sample distribution')
...
Setting.K = [128, 128, 128]  # len(np.unique(train_label))
...
Theta_concat = tf.concat([u_1 * Theta_1, u_2 * Theta_2, u_3 * Theta_3], axis=0)
...
for sweepi in range(Setting.SweepTimes):
    ...
    if np.mod(sweepi, 1) == 0:
        ...
        # prediction metric
        try:
            AUC_score, AP_score = get_roc_score(test_edges, test_edges_false, adj_orig, emb=Theta_concat_t.T)
            predict_test_acc = get_acc(test_edges, test_edges_false, emb=Theta_concat_t.T)
        except:
