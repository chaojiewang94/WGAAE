...
def preprocess_graph4sample(adj):
...
def construct_feed_dict(adj_normalized, adj, placeholders):
...
def node_sampling(adj, distribution, nb_node_samples, k, replace=True):
    ...
    sampled_nodes = np.random.choice(adj.shape[0], size = int(k*nb_node_samples),
                                     replace = replace, p = distribution)
    #one = np.ones(shape=distribution)
    distribution_r = (1.0 - distribution) / (adj.shape[0] - 1.0)
    sampled_nodes_r = np.random.choice(adj.shape[0], size =nb_node_samples - int(k*nb_node_samples),
                                     replace = replace, p =distribution_r)
    sampled_nodes = np.hstack((sampled_nodes,sampled_nodes_r))
